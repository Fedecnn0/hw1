...
function aggpreferito(event){
    ...
    fetch("aggpreferiti.php?dati="+breed+"&foto="+img+"&razza="+un);
}
...
function rempreferito(event){
    ...
    fetch("rempreferiti.php?dati="+breed+"&foto="+img+"&razza="+un);
    console.log(breed);
}
...
function searchForBreed(event){
    ...
    fetch("controllo.php?breed="+breed).then(onRespBreed).then(onJsonBreed2);
    fetch("ricerca.php").then(onRespBreed).then(onJsonBreed);
}
...
let img;
let un;
fetch("https://api.thedogapi.com/v1/breeds").then(onResponse).then(onJson);
